...
namespace BitStreams
{
    ...
    public class BitStream
    {
        ...
        {
            if (!File.Exists(path)) throw new IOException("File doesn't exists!");
            if (File.GetAttributes(path) == FileAttributes.Directory) throw new IOException("Path is a directory!");
        ...
        public void CutStream(long offset, long length)
        {
            ...
            Array.Copy(data, (int) offset, buffer, 0, (int) length);
            _stream = new MemoryStream();
            var m = new MemoryStream(buffer);
            _stream = new MemoryStream();
            m.CopyTo(_stream);
        ...
        public Bit ReadBit()
        {
            if (!ValidPosition) throw new IOException("Cannot read in an offset bigger than the length of the stream");
            _stream.Seek(Offset, SeekOrigin.Begin);
            byte value;
            if (!Msb)
        ...
        public Bit[] ReadBits(int length)
        {
            ...
            for (var i = 0; i < length; i++) bits[i] = ReadBit();
            return bits;
        }
        ...
        {
            foreach (var b in bits) WriteBit(b);
        }
        ...
        public void WriteBits(ICollection<Bit> bits, int length)
        {
            ...
            bits.CopyTo(b, 0);
            for (var i = 0; i < length; i++) WriteBit(b[i]);
        }
        ...
        {
            for (var i = offset; i < length; i++) WriteBit(bits[i]);
        }
        ...
        {
            return _encoding.GetChars(ReadBytes(_encoding.GetMaxByteCount(1) * 8))[0];
        }
        ...
        {
            var bitsPerChar = _encoding.GetByteCount(" ") * 8;
            return _encoding.GetString(ReadBytes(bitsPerChar * length));
        }
        ...
        {
            var bytes = _encoding.GetBytes(new[] {value}, 0, 1);
